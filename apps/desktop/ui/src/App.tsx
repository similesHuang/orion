          ...
          <div className="input-area">
            ...
            {session?.pendingFiles.length > 0 && (
              <div className="pending-files">
                <div className="pending-list">
                  {session.pendingFiles.map((file, index) => (
                    <span key={`${file}-${index}`} className="file-chip">
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (!session) return
                          dispatch({ type: 'removePendingFile', sessionId: session.id, index })
                        }}
                      >
                        ×
                      </button>
            ...
            <div className="input-toolbar">
              <div className="toolbar-left">
                <div className="attach-menu" ref={attachPopoverRef}>
                  <button
                    className={`attach-trigger ${attachOpen ? 'active' : ''}`}
                    title="更多"
